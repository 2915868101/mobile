    ...
    <div class="s-bgcF4F5FA" ref="echart1">
      <div class="f-ml12 f-mr12 f-mt10 s-bgcfff">
        <div class="title-icon-wrap f-df f-dfj" @click="doToggle('echart1')">
          <div class="f-fs16"> <i class="wsdfont wsd-zhuzhuangtushujutu f-lh24 f-pr5"></i>{{classid===0?'各机构医疗总费用统计':'各机构住院医疗费用统计'}}</div>
    ...
    <div class="s-bgcF4F5FA" ref="table2">
      <div class="f-ml12 f-mr12 f-mt10 s-bgcfff">
        ...
        <el-table :data="tableData2" :max-height="isFullPage?350:getTableMaxWidth(50)" stripe :header-cell-style="{background:'#FAFAFA',color:'#283142'}">
          ...
          <el-table-column align="center" prop="value3" :label="[classid===0?'门急诊医疗费用(万元)':'住院人次(次)']"></el-table-column>
          <el-table-column align="center" prop="value4" :label="[classid===0?'住院医疗费用(万元)':'住院手术费用(万元)']"></el-table-column>
          <el-table-column align="center" prop="value5" :label="[classid===0?'医保总费用(万元)':'住院药品费用(万元)']"></el-table-column>
          <el-table-column align="center" prop="value6" :label="[classid===0?'门急诊次均费用(元)':'住院检查费用(万元)']"></el-table-column>
          <el-table-column align="center" prop="value7" :label="[classid===0?'住院次均费用(元)':'住院检验费用(万元)']"></el-table-column>
          <el-table-column align="center" prop="value8" :label="[classid===0?'门急诊药品费用(万元)':'住院其他费用(万元)']"></el-table-column>
          <el-table-column align="center" prop="value9" :label="[classid===0?'门急诊药费占比(%)':'住院总费用(万元)']"></el-table-column>
          <el-table-column align="center" prop="value10" label="住院药品费用(万元)" v-if="classid===0"></el-table-column>
          <el-table-column align="center" prop="value11" label="住院药费占比(%)" v-if="classid===0"></el-table-column>
          <el-table-column align="center" prop="value12" label="医疗总费用(万元)" v-if="classid===0"></el-table-column>
        </el-table>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      if (t.$route.query.name === "医疗总费用本期" || t.$route.query.name === "医疗总费用同期") {
        t.classid = 0
        t.title = "医疗总费用"
      } else {
        t.classid = 1
    ...
    methods: {
      ...
      getZyMrcFyTbHbTypeInfo() {
        ...
        t.ajax({
          methods: 'post',
          url: t.WSD.serverPath1 + 'mobile/getYlbzYyxl',
          params: {
            TableId: t.classid,
