    ...
    <div class="aa f-pl12 f-pr12 f-pb5">
      <div class="date-wrap">
        <div class="f-pt12 f-pb12 f-pr10 f-df f-dfj">
          ...
          <el-button type="primary" color="#409EFF" @click="doSelect()">查询</el-button>
        </div>
      ...
      <template>
        <div class="f-mb10">
          <van-cell :value="tableData1.NAME1+'例'">
            <template #title>
              <div class="icon f-fl wsdfont wsd-yuandianf"></div>
              <div class="f-ml15">传染病及公卫事件报告数(例)</div>
            </template>
          </van-cell>
          <van-cell :value="tableData1.NAME2">
            <template #title>
              <span class="icon f-fl wsdfont wsd-yuandianf"></span>
              <div class="f-ml15">传染病疫情报告率(%)</div>
            </template></van-cell>
          <van-cell :value="tableData1.NAME3"> <template #title>
              <span class="icon f-fl wsdfont wsd-yuandianf"></span>
              <div class="f-ml15">传染病疫情报告及时率(%)</div>
            </template></van-cell>
          <van-cell :value="tableData1.NAME4"> <template #title>
              <span class="icon f-fl wsdfont wsd-yuandianf"></span>
              <div class="f-ml15">公共事件信息报告率(%)</div>
            </template>
          </van-cell>
        </div>
        ...
        <div class="screen-wrap" v-if="tableData" ref="table1">
          ...
          <div class="f-mt8 f-mb8 f-pr">
            <el-table :data="tableData" ref="tableData" max-width="100%" :max-height="isFullPage?getTableMaxWidth(50):getTableMaxHeight(50)" stripe :header-cell-style="{background:'#FAFAFA',color:'#283142'}">
              ...
              <el-table-column align="center" prop="VALUE2" label="传染病及公卫事件报告数"></el-table-column>
              <el-table-column align="center" prop="VALUE3" label="传染病报告数"></el-table-column>
              <el-table-column align="center" prop="VALUE4" label="公卫事件报告数"></el-table-column>
            </el-table>
          </div>
        </div>
      </template>
...
<script>
  export default {
    ...
    created() {
      var t = this
      t.updateTitle('传染病及突发公共卫生事件报告和处理')
    ...
    methods: {
      ...
      getQyKsZbInfo() {
        ...
        t.ajax({
          methods: 'post',
          url: t.WSD.serverPath1 + 'mobile/getGWCrbtfbghcl1',
          params: t.searchForm,
...
<style scoped>
  ...
  .zyzb_zyyzb>>>.el-table__header-wrapper {
    z-index: 30;
  }

  .zyzb_zyyzb>>>.el-table__body-wrapper {
    z-index: 21;
    position: relative;
  }

  .zyzb_zyyzb>>>.el-table__fixed {
    z-index: 41;
  }

  .zyzb_zyyzb>>>.el-table__footer-wrapper {
    z-index: 31;
  }
