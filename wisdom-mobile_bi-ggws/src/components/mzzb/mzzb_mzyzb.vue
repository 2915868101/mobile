<template>
  <div class="mzzb_mzyzb g-wrap">
    <div class="aa f-pl12 f-pr12 f-pb5">
      ...
      <template>
        <div class="f-mb10">
          <van-cell :value="tableData1.NAME1+'次'">
            <template #title>
              <span class="icon f-fl wsdfont wsd-yuandianf"></span>
              <div class="f-ml15">健康教育活动开展次数(次)</div>
            </template>
          </van-cell>
          <van-cell :value="tableData1.NAME2+'人'">
            <template #title>
              <span class="icon f-fl wsdfont wsd-yuandianf"></span>
              <div class="f-ml15">健康教育参与总人数(人)</div>
            </template></van-cell>
          <van-cell :value="tableData1.NAME3+'次'"> <template #title>
              <span class="icon f-fl wsdfont wsd-yuandianf"></span>
              <div class="f-ml15">常驻人口人次均数(次)</div>
            </template></van-cell>
          <van-cell :value="tableData1.NAME4+'份'"> <template #title>
              <span class="icon f-fl wsdfont wsd-yuandianf"></span>
              <div class="f-ml15">资料发放数量(份)</div>
            </template>
          </van-cell>
        </div>
        ...
        <div class="screen-wrap" v-if="tableData" ref="table1">
          ...
          <div class="f-mt8 f-mb8 f-pr">
            <el-table :data="tableData" ref="tableData" max-width="100%" :max-height="isFullPage?getTableMaxWidth(50):getTableMaxHeight(50)" stripe :header-cell-style="{background:'#FAFAFA',color:'#283142'}">
              ...
              <el-table-column align="center" prop="VALUE2" label="资料发放数(份)"></el-table-column>
              <el-table-column align="center" prop="VALUE3" label="参与总人数"></el-table-column>
              <el-table-column align="center" prop="VALUE4" label="参与人次均份数(份)"></el-table-column>
            </el-table>
          </div>
        </div>
      </template>
  ...
  export default {
    ...
    methods: {
      ...
      getQyKsZbInfo() {
        ...
        t.ajax({
          methods: 'post',
          url: t.WSD.serverPath1 + 'mobile/getGWJkjy1',
          params: t.searchForm,
...
<style scoped>
  ...
  .mzzb_mzyzb .date-wrap .date-input {
    width: 41%;
  ...
  .mzzb_mzyzb .date-wrap .date-input i {
  ...
  .mzzb_mzyzb>>>.van-button {
    height: 32px;
  }

  .mzzb_mzyzb>>>.el-table__header-wrapper {
    z-index: 30;
  }

  .mzzb_mzyzb>>>.el-table__body-wrapper {
    z-index: 21;
    position: relative;
  }

  .mzzb_mzyzb>>>.el-table__fixed {
    z-index: 41;
  }

  .mzzb_mzyzb>>>.el-table__footer-wrapper {
    z-index: 31;
  }
