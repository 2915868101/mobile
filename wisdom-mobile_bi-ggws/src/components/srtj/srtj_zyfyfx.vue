        ...
        <div class="screen-wrap" v-if="tableData" ref="table1">
          ...
          <div class="f-mt8 f-mb8 f-pr">
            <el-table :data="tableData" ref="tableData" max-width="100%" :max-height="isFullPage?getTableMaxWidth(50):getTableMaxHeight(50)" stripe :header-cell-style="{background:'#FAFAFA',color:'#283142'}">
              <el-table-column align="center" prop="VALUE1" fixed label="医疗机构"></el-table-column>
              <el-table-column align="center" prop="VALUE2" label="新生儿访视率(%)"></el-table-column>
              <el-table-column align="center" prop="VALUE3" label="辖区活产数"></el-table-column>
              <el-table-column align="center" prop="VALUE4" label="新生儿访视人数"></el-table-column>
            </el-table>
          </div>
        </div>
      </template>
  ...
  export default {
    ...
    methods: {
      ...
      getQyKsZbInfo() {
        ...
        t.ajax({
          methods: 'post',
          url: t.WSD.serverPath1 + 'mobile/getGWRtbj',
          params: t.searchForm,
...
<style scoped>
  ...
  .srtj_zyfyfx>>>.el-table__header-wrapper {
    z-index: 30;
  }

  .srtj_zyfyfx>>>.el-table__body-wrapper {
    z-index: 21;
    position: relative;
  }

  .srtj_zyfyfx>>>.el-table__fixed {
    z-index: 41;
  }

  .srtj_zyfyfx>>>.el-table__footer-wrapper {
    z-index: 31;
  }
