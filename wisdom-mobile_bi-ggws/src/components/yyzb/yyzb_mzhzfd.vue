    ...
    <div class="table-wrap s-bgcfff" v-if="tableData1" ref="table1">
      ...
      <el-table :data="tableData1" :max-height="isFullPage?getTableMaxHeight(50):250" border>
        ...
        <el-table-column prop="value" align="center" label="本期值" min-width="20">
          <template slot-scope="scope">
            <div style="text-align: right;">{{parseFloat(scope.row.value1)}}</div>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    //门诊患者负担分析
    getMzMrcFyTbHbTypeInfo() {
      var t = this
      t.ajax({
        methods: 'post',
        url: t.WSD.serverPath1 + 'mobile/getMzMrcFyTbHbTypeInfo',
        params: t.searchForm,
    ...
    //每人次费用
    getMzMrcFyTypeInfoTop() {
      var t = this
      t.ajax({
        methods: 'post',
        url: t.WSD.serverPath1 + 'mobile/getMzMrcFyTypeInfoTop',
        params: t.searchForm,
        success: function(data) {
          t.$nextTick(function() {
    ...
    getMzMrcFyTypeInfo() {
      ...
      t.ajax({
        methods: 'post',
        url: t.WSD.serverPath1 + 'mobile/getMzMrcFyTypeInfo',
        params: t.searchForm,
        success: function(data) {
          t.tableData2 = data || []
        }
