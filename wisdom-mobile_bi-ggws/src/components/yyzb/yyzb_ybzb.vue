      ...
      <template>
        <div class="f-mb10">
          ...
          <van-cell :value="tableData1.NAME3+'次'"> <template #title>
              <span class="icon f-fl wsdfont wsd-yuandianf"></span>
              <div class="f-ml15">已随访人次(次)</div>
            </template></van-cell>
          <van-cell :value="tableData1.NAME4"> <template #title>
              <span class="icon f-fl wsdfont wsd-yuandianf"></span>
              <div class="f-ml15">规则服药率(%)</div>
            </template>
          </van-cell>
        </div>
        ...
        <div class="screen-wrap" v-if="tableData" ref="table1">
          ...
          <div class="f-mt8 f-mb8 f-pr">
            <el-table :data="tableData" ref="tableData" max-width="100%" :max-height="isFullPage?getTableMaxWidth(50):getTableMaxHeight(50)" stripe :header-cell-style="{background:'#FAFAFA',color:'#283142'}">
              ...
              <el-table-column align="center" prop="VALUE2" label="建档总数(份)"></el-table-column>
            </el-table>
          </div>
        </div>
      </template>
  ...
  export default {
    ...
    methods: {
      ...
      getQyKsZbInfo() {
        ...
        t.ajax({
          methods: 'post',
          url: t.WSD.serverPath1 + 'mobile/getGWFjehz1',
          params: t.searchForm,
...
<style scoped>
  ...
  .yyzb_ybzb>>>.el-table__header-wrapper {
    z-index: 30;
  }

  .yyzb_ybzb>>>.el-table__body-wrapper {
    z-index: 21;
    position: relative;
  }

  .yyzb_ybzb>>>.el-table__fixed {
    z-index: 41;
  }

  .yyzb_ybzb>>>.el-table__footer-wrapper {
    z-index: 31;
  }
